...
public class GAMEVIEW {
    ...
    private void initColormap() {
        ...
        // Eingefügt von Tobias Hassebrock
        colormap.put('T', Color.GRAY);
    }
     ...
     */
    public void playSound(String sound, boolean replay) {
        this.sound.playSound(sound, replay);
    }
    ...
    private static class Mouse {
        ...
        private final TimerTask invisibleMouseTimerTask;
    ...
    private static class Sound {
        ...
        void playSound(String sound, boolean replay) {
    ...
    private static class TextPanel extends JPanel {
        ...
        private int height;
        private int width;
        private final Map<TextAttribute, Object> fontMap;
        private Font font;
        ...
        protected void paintComponent(Graphics g) {
            this.painting = true;
            if (!canvasConsumed) {
                Canvas canvasToPaint = this.canvasFromWindow;
                this.canvasConsumed = true;
                boolean structureIsNew;
                ...
                for (int i = 0; i < canvasToPaint.getLines(); i++) {
                    for (int j = 0; j < canvasToPaint.getColumns(); j++) {
                        ...
                        if (structureIsNew || isDifferent(canvasToPaint, lastPaintedCanvas, i, j) || (j > 0 && isDifferent(canvasToPaint, lastPaintedCanvas, i, j - 1)) || (i > 0 && isDifferent(canvasToPaint, lastPaintedCanvas, i - 1, j))) {
                            int yForRect = i * lineHeight;
                            // Variablen zur Optimierung nicht lokal
                            int xForBoth = j * charWidth;
                            int yForString = yForRect + (int) Math.rint(lineHeight * 0.75);
